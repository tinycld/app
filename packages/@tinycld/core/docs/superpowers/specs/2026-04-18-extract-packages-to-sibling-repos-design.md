# Extract Packages to Sibling Repos (pnpm link)
...
- Git submodules. We considered them and picked `pnpm link` instead because it has no git coupling between core and packages.
...
A proof of concept (2026-04-18) confirmed that `pnpm link` works end-to-end against the generator with zero code changes: `import.meta.resolve` (generate-packages.ts:108-123) follows the link, typecheck passes, and live edits in the sibling repo are visible through `node_modules/@tinycld/<pkg>` immediately.
...
2. **`tinycld.packages.ts`:** stays as the source of truth for what gets wired in. By default on a fresh clone it is **empty** (`export const packages = [] as const`). Cloning core and running `pnpm install && pnpm run dev` gives you a working shell with no packages, no errors.
...
### New helper: `pnpm run packages:link`

A thin wrapper in `scripts/link-package.ts` for the common dev flow. Invoked as:

```sh
pnpm run packages:link contacts           # link ../contacts into core
pnpm run packages:link contacts ../../elsewhere/contacts   # custom path
pnpm run packages:unlink contacts         # remove the link
```
...
3. `cd <sibling> && pnpm link` to register the global link (idempotent).
4. Remove any existing `node_modules/@tinycld/<slug>` in core (the workspace or stale link, if present).
5. `cd core && pnpm link @tinycld/<slug>` to install the link.
6. Add `@tinycld/<slug>` to `tinycld.packages.ts` if not already listed.
7. Run `pnpm run packages:generate` to wire everything up.
...
3. `cd <sibling> && pnpm unlink` (the only form of `pnpm unlink` that works in 1.3.12 — there's no `pnpm unlink <pkg>`).
4. Run `pnpm run packages:generate` to clean up routes/migrations/registry for the removed package.
...
cd core
pnpm install
pnpm run dev                    # works, shows the shell with no packages

# elsewhere:
mkdir ../example-pkg && cd ../example-pkg
# scaffold a package (package.json with name @tinycld/example, manifest.ts, etc.)
cd ../core
pnpm run packages:link example ../example-pkg
pnpm run dev                    # now has the example package wired in
```
...
CI job (or deployment script) clones core, clones the two package repos it wants, runs `packages:link` for each, then `pnpm run build:web`. The resulting bundle contains only those two packages. Calendar and drive are never cloned, never installed, never referenced.

### What about the workspace-style flow we have today?

Gone. After this change, `packages/*` inside core doesn't exist as a thing bun knows about. Developers who want "one repo, one `pnpm install`, everything works" can still get that experience — they just run the link commands once and they persist in `bun`'s global link registry. The `tinycld.packages.ts` file records which packages are *wired in*; the global link registry records which are *resolved from siblings*. A package must be in both to work.
...
3. **Extract `@tinycld/contacts`** first (smallest, fewest cross-package dependencies):
   ...
   - In core: `pnpm run packages:link contacts` pointing at the new sibling; run full check suite (`pnpm run checks`, `pnpm run test:unit`, `pnpm run test:e2e` for contacts).
...
- **`pnpm install` overwrites a link:** documented behavior — the recommended order is `pnpm install` first, then link. If a user hits it, `packages:link` is idempotent, so re-running it fixes the state.
...
- **Sibling MUST NOT have its own `node_modules/`.** bun auto-installs peer dependencies, so `pnpm install` inside a sibling package creates a duplicate of every peer (`react`, `react-native`, `pbtsdb`, `@tanstack/db`, …). Under the `pnpm link` symlink, TypeScript sees two copies of every type and fails with a flood of "Type X is not assignable to type X" errors. Each sibling package repo should commit a `.gitignore` rule for `node_modules/` AND `pnpm-lock.yaml` (a tracked lockfile invites future `pnpm install` runs, which recreate `node_modules/`) — and skip `pnpm install` during dev. The sibling gets its peer deps through core's `node_modules/` via the symlink. If the sibling does need to install something for its own tests, use `pnpm install --no-peer` (bun does not expose this flag today — workaround: delete `node_modules/` after install). This is a real operational constraint, not a nice-to-have.
...
- E2E smoke test: CI job that clones core, links contacts from a sibling fixture, runs `pnpm run dev` long enough to hit the contacts list page, asserts the server returns 200.
