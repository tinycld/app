...
function ColorSwatchRow({
    ...
    return (
        <View className="flex-row flex-wrap justify-around px-3 pb-2">
            {LABEL_COLORS.map(c => {
    ...
    if (isEditing) {
        return (
            <View style={{ backgroundColor: surfaceBg }}>
                ...
                <ColorSwatchRow selected={editColor} onSelect={c => setValue('color', c)} />
            </View>
        )
    }
...
function CreateLabelRow({ onCreated }: { onCreated: () => void }) {
    ...
    return (
        <View style={{ backgroundColor: surfaceBg }}>
            ...
            <ColorSwatchRow selected={color} onSelect={c => setValue('color', c)} />
        </View>
    )
}
