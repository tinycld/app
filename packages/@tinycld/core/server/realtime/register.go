...
func handleConnect(broker *Broker, opts Options, re *core.RequestEvent) error {
	...
	conn, err := websocket.Accept(re.Response, re.Request, &websocket.AcceptOptions{
		// Same-origin only; the app and the API share the host.
	...
	go runConnection(broker, opts, kind, roomID, conn)
	return nil
}
...
func runConnection(broker *Broker, opts Options, kind, roomID string, conn *websocket.Conn) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	client := &Client{joinedAt: time.Now()}
